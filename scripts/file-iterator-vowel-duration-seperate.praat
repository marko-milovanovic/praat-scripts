...
for file_index to number_of_files
	...
	for vowelIndex from 1 to numberOfVowels
		for columnIndex from 1 to numberOfColumns
        	sumDuration[columnIndex] = 0
        	count[columnIndex] = 0
			durations[columnIndex] = undefined
        	minDuration[columnIndex] = 1e9
        	maxDuration[columnIndex] = 0
    	endfor
			...
			if phonemeLabel$ <> "" and index (" " + unwantedPhonemes$ + " ", " " + phonemeLabel$ + " ") == 0
				...
				for columnIndex from 1 to numberOfColumns
					if index (" " + phonemeMap$[vowelIndex,columnIndex] + " ", " " + phonemeLabel$ + " ") > 0
						sumDuration[columnIndex] += durationMiliseconds
						count[columnIndex] += 1
						durations[columnIndex,count[columnIndex]] = durationMiliseconds
						if durationMiliseconds < minDuration[columnIndex]
							minDuration[columnIndex] = durationMiliseconds
						endif
						if durationMiliseconds > maxDuration[columnIndex]
							maxDuration[columnIndex] = durationMiliseconds
						endif
					endif
				endfor
			endif
		...
		outputPath$ = "'directory_output_path$''file_name$'_'vowel$'.txt"
		header$ = columnNames$
		...
		for columnIndex from 1 to numberOfColumns
			averageDuration = sumDuration[columnIndex] / count[columnIndex]

			standardDeviation = 0
			for index from 1 to count[columnIndex]
        		standardDeviation += (durations[columnIndex,index] - averageDuration)^2
    		endfor
    		standardDeviation = sqrt(standardDeviation / (count[columnIndex] - 1))
			...
			minDurationLine$ = minDurationLine$ + separator$ + string$(minDuration[columnIndex])
			maxDurationLine$ = maxDurationLine$ + separator$ + string$(maxDuration[columnIndex])
		endfor
